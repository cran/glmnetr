...
#' analysis. Note, in modifications for 
#' the elastic net including alpha as a tuning parameter, the current 
#' nested.glmnetr() version uses cv.glmnet() for the elastic net calculations and 
#' calucaltions may yield slight differnt numbers for the relaxes lasso.   
#' 
...
#' elements $epochs, $epochs2, $myler, $myler2, $eppr, $eppr2, $lenz1, $lenz2, 
...
#' @param dofull  fit and do cross validation for the full model including all 
#' terms (without a penalty), 0 or 1. This is provided primarily as a reference. 
...
#' @param limit Formally to limit the small values for lambda after the initial 
#' fit. Note, the current 
#' nested.glmnetr() version uses cv.glmnet() for the elastic net calculations 
#' and the limit paramter is disregarded.   
#' @param fine  Formally used for a finer step in determining lambda. 
#' Of little value unless one repeats the cross validation many times to more 
#' finely tune the hyper paramters. See the 'glmnet' package documentation. Not 
#' used for current version of nested.glmnetr which uses cv.glmnetr() for 
#' relaxed lasso model fits.    
#' @param ties method for handling ties in Cox model formally used for the 
#' relaxed model component. In modifications for 
#' the elastic net including alpha as a tuning parameter, the current 
#' nested.glmnetr() version uses cv.glmnet() for the elastic net calculations 
#' which does not allow use of the Efron approach to handling ties, even for the 
#' gamma = 0 fits. 
...
#' @param path - as in glmnet(). default is 0. glmnet() can have 
#' troubles converging for logistic and Cox models in which case one can set 
#' path = 1. When not needed path = 1 generally has longer computation times. 
#' In modifications for the elastic net including alpha as a tuning paramger, 
...
nested.glmnetr = function(xs, start=NULL, y_, event=NULL, family="gaussian", resample=NULL, folds_n=10, stratified=NULL,  
                          dolasso=1, doxgb=0, dorf=0, doorf=0, doann=0, dorpart=0, dostep=0, doaic=0, dofull=0, 
                          ensemble=0, method="loglik", alpha=NULL, gamma=NULL, lambda=NULL, steps_n=0,
                          seed=NULL, foldid=NULL, limit=1, fine=0, ties="efron", keepdata=0, keepxbetas=1,  
                          bootstrap=0, unique=0, id=NULL, track=0, do_ncv=NULL, int_file=NULL, path=0, ...  ) {
  ...
  gamma = unique(gamma)
  gamma = sort(gamma)
  if (is.null(alpha)) { alpha = c(1) }                                        ## alpha 1 for lasso, alpha 0 for ridge 
  alpha = alpha[((alpha >= 0) & (alpha <= 1))]
#  if (!(0 %in% alpha)) { alpha = c(0,alpha)}
  if (!(1 %in% alpha)) { alpha = c(alpha,1) }
  if (length(alpha) > 1) { 
#    if (!(0 %in% alpha)) { alpha = c(0,alpha)}
    }
  alpha = sort(alpha)
...
#  print( runif(1) ) 
  ...
  ones_nby4 = matrix ( data=rep(1,(4*reps)), nrow = reps, ncol = 4 )
  zero_nby4 = matrix ( data=rep(0,(4*reps)), nrow = reps, ncol = 4 )
  step.devian.rep = ones_nby4
  step.cal.devian.rep = ones_nby3
  step.intcal.rep = zero_nby4
  step.lincal.rep = zero_nby4
  step.agree.rep  = zero_nby4
  step.nzero.rep  = zero_nby4
  step.p.rep      = matrix ( data=rep(0,(reps)), nrow = reps, ncol = 1 )
  step_nms = c("df", "p", "AIC", "Full") 
  step_xb_nms = c("step.df", "step.p", "aic", "full")
  colnames(step.devian.rep)     = step_nms
  colnames(step.cal.devian.rep) = step_nms[c(1:3)]
...
#    cat(paste(" (2) fine = ", fine, "\n"))
    ...
    cv_lasso_fit_f   = cv.glmnetr.list$cv_lasso_fit 
    cv_ridge_fit_f   = cv.glmnetr.list$cv_ridge_fit   
    cv_elastic_fit_f = cv.glmnetr.list$cv_elastic_fit
    lambda_null      = cv.glmnetr.list$lambda_null
    lambda_1         = cv_lasso_fit_f$lambda

#    class(cv_lasso_fit_f)
#    print( cor(xbetas[,2:6]) ) 
...
#    testxs=xs ; testy__=y__ ; trainxs=xs ; trainy__=y__ ; tol=1e-14 ;
    ...
    lasso.intcal.naive      = cv_glmnetr_perf$lasso.intcal
    lasso.lincal.naive      = cv_glmnetr_perf$lasso.lincal
    lasso.nzero             = cv_glmnetr_perf$lasso.nzero 
    lasso.cal.devian.naive  = cv_glmnetr_perf$lasso.cal.devian
    xbetas.lasso            = cv_glmnetr_perf$xbetas.lasso 
    ofst                    = cv_glmnetr_perf$ofst 
...
#      print( rbind( rf_perf1, rf_perf1_ ))
    ...
    if ( (sum(ensemble[c(3,4,7,8)]) >= 1) & (family == "gaussian") ) { 
      rfmods = rfmods + 1 
      if (track >= 2) { cat( "     Relaxed lasso predicteds as offset\n", "    event is NULL", is.null(event), "    family = ", family, "\n") }
      yo = y_ - ofst                                                            ## 231228 
      rf_tunedO    = rf_tune(xs=xs, y_=yo, event=event, family=family, mtryc=mtryc, ntreec = ntreec, nsplitc=nsplitc, seed=seedr_)
      predm3       = rf_xbhat(rf_model=rf_tunedO$rf_tuned, dframe=as.data.frame(xs) , ofst=as.numeric(ofst) , family=family, tol=tol_, track=track) 
...
#    if (track >= 2) { cat("\n") }
  ...
  if (doorf_==1) { 
    ...
    if (sum(ensemble[c(2,6)]) >= 1) { 
      orfmods = orfmods + 1 
      if (track >= 2) { cat( "     Relaxed lasso predicteds as feature\n") }
      xsO = cbind(xs, as.matrix(ofst))
      colnames(xsO)[dim(xsO)[2]] = "ofst"
      orf_tunedF = orf_tune(xs=xsO, y_=y_, event=event, family=family, mtryc=omtryc, ntreec = ontreec, nsplitc=onsplitc, seed=seedr_, track=track) 
      predm2       = orf_xbhat(orf_model=orf_tunedF$orf_tuned, as.data.frame(xsO) , ofst=NULL, family=family, tol=tol_) 
      predm2.cal   = cal_train_xbhat( predm2 , y__ , predm2 , family ) 
...
#    if (track >= 2) { cat("\n") }
  ...
  if (doann_ == 1) { 
    ...
    if (sum(ensemble[c(2:8)]) > 0) {
      lassopred0 = predict(cv_lasso_fit_f, xs, lam="lambda.min" , gam="gamma.min", comment = 0)
      lassobeta  = predict(cv_lasso_fit_f,     lam="lambda.min" , gam="gamma.min", comment = 0 )
      if (family=="cox") {
    ...
    getl1 = 0 ; getl12 = 0 ; 
    if (l1  < 0) { getl1  = 1 ; l1  = abs(l1 ) * cv_lasso_fit_f$relaxed$lambda.min.g0 } 
    if (l12 < 0) { getl12 = 1 ; l12 = abs(l12) * cv_lasso_fit_f$relaxed$lambda.min.g0 }
    ...
    keepxbetas.ann = c(1:8)[ (ensemble[1:8]==1) ] 
...
#    print(" HERE 1") ; print(xbetas.cv.ann[1,])
  ...
  ##### STEPWISE, AIC and FULL fits ############################################################################ 
  
  if (dofull==1) {
    if (length(y_) <= (dim(xs)[1] * floor((folds_n -1)/folds_n)) ) { dofull = 0 }
  }
  
  if ((dostep == 1) | (doaic == 1) | (dofull == 1)) {  
    step.devian.naive = c(1,1,1,1)
    step.cal.devian.naive = c(1,1,1)
    step.agree.naive   = c(0,0,0,0)
#    step.intcal.naive  = c(0,0,0,0)
#    step.lincal.naive  = c(0,0,0,0)
    step.nzero         = c(0,0,0,0)
    names(step.devian.naive) = step_nms 
    names(step.cal.devian.naive) = step_nms[c(1:3)] 
    names(step.agree.naive ) = step_nms
    names(step.nzero       ) = step_nms
    
    keepxbetas.step = c(0,0,0,0) 
    if (dostep == 1) { keepxbetas.step[c(1,2)] = c(1,2) }
    if (doaic  == 1) { keepxbetas.step[c(3)] = 3 }
    if (dofull == 1) { keepxbetas.step[c(4)] = 4 }
    xbetas.step = matrix(rep(xbnull,4*nobs), nrow=nobs, ncol=4)
    xbetas.cv.step = matrix(rep(xbnull,4*nobs), nrow=nobs, ncol=4)
  ...
  if (doaic ==1) { 
    ...
    aic_perf1 = perf_gen( y__ , predaic   , family )                            ## aic_perf1 to distinguish from aic_perf()
  ...
  ##### Full model fit #################################################################################################  
  if (dofull == 1) {
    if (track >= 1) { cat("\n ########## Initial full model fit of all data #####################################\n") } 
    if (family == "cox") {
      if (is.null(start)) { 
        full_fit = coxph(Surv(y_,event) ~ . , data = data.frame(cbind(xs, y_, event)) )
        predfull = predict(full_fit, newdata = data.frame(cbind(xs, y_, event)) )
      } else { 
        full_fit = coxph(Surv(start, y_, event) ~ . , data = data.frame(cbind(xs, start, y_, event)) ) 
        predfull = predict(full_fit, newdata = data.frame(cbind(xs, start, y_, event)) )
      }
    } else { 
      full_fit = glm( y_ ~ xs )
      predfull = predict(full_fit, data = data.frame(cbind(xs, y_)) )
    } 
    full_perf = perf_gen( y__ , predfull , family=family )
    full_perf 
    
    step.devian.naive[c(4)]     = c( full_perf[1] )
    step.agree.naive[c(4)]      = c( full_perf[3] )
    step.nzero[4]  = sum(!is.na(full_fit$coefficients))
    
    xbetas.step[,4] = predfull
    
    if (track >= 1) { time_last = diff_time(time_start, time_last) } 
  }
  
  if ((dostep == 1) | (doaic == 1) | (dofull==1)) { xbetas = cbind(xbetas, xbetas.step[,keepxbetas.step, drop=FALSE]) }
  ...
  if (!is.null(int_file)) { 
    nestedcv = nested_tolist(Call, family, xs, start, y_, y__, event, ties, id, start_name, y_name, event_name,  
                            seed, foldid, ensemble, folds_n, alpha, gamma, lambda, stratified, limit, fine, method, steps_n, 
                            bootstrap, bootstrap_, unique, resample, keepdata, keepxbetas, track, 
                            dolasso, doxgb, doxgb_, dorf, dorf_, doorf, doorf_, dorpart, doann, doann_, dostep, doaic, dofull, 
                            ...
                            lasso.nzero, lasso.cal.devian.naive,  
                            cv_lasso_fit_f, cv_ridge_fit_f, cv_elastic_fit_f, cv_glmnet_fit_f_, 
                            ...
                            cv.stepreg.fit.all, cv.stepreg.fit , func.fit.aic , full_fit , 
...
#      print(null.m2LogLik.rep)
      ...
      if (dolasso==1) { 
        ...
        cv.glmnetr.list = cv.glmnetr(trainxs, trainy__, family, lambda=lambda, gamma=gamma, alpha=alpha, foldid=foldidcv, folds_n=folds_n, fine=fine, path=path, track=track, ... ) 
        ...
        cv_glmnetr_perf = cv.glmnetr_perf(cv.glmnetr.list, testxs, testy__, trainxs, trainy__, family, ensemble, lasso_nms, lasso_xb_nms )
        lasso.devian.rep[i_,]     = cv_glmnetr_perf$lasso.devian
        lasso.agree.rep[i_,]      = cv_glmnetr_perf$lasso.agree
        lasso.intcal.rep[i_,]     = cv_glmnetr_perf$lasso.intcal
        lasso.lincal.rep[i_,]     = cv_glmnetr_perf$lasso.lincal
        lasso.nzero.rep[i_,]      = cv_glmnetr_perf$lasso.nzero 
        xbetas.lasso.tmp          = cv_glmnetr_perf$xbetas.lasso 
        ofst                      = cv_glmnetr_perf$ofst 
        
        lasso.cal.devian.rep[i_,] = cv_glmnetr_perf$lasso.cal.devian
        lasso.cal.devian.rep[i_,] = cv_glmnetr_perf$lasso.cal.devian
        lasso.cal.agree.rep[i_,]  = cv_glmnetr_perf$lasso.cal.agree
        lasso.cal.intcal.rep[i_,] = cv_glmnetr_perf$lasso.cal.intcal
        lasso.cal.lincal.rep[i_,] = cv_glmnetr_perf$lasso.cal.lincal
          
        ## store predicteds 
        xbetas.lasso.tmp = cv_glmnetr_perf$xbetas.lasso 
        
        if (bootstrap == 0) {
          xbetas.cv.lasso[ oob ,] = xbetas.lasso.tmp   
          colnames(xbetas.cv.lasso) = lasso_xb_nms 
        } else if (bootstrap == 1) {
          xbetas.lasso.tmp.tr = cv_glmnetr_perf$xbetas.lasso.tr 
          if (i_ == 1) {
            xbetas.boot.oob.lasso = xbetas.lasso.tmp 
            xbetas.boot.inb.lasso = xbetas.lasso.tmp.tr 
            colnames(xbetas.boot.oob.lasso) = lasso_xb_nms 
            colnames(xbetas.boot.inb.lasso) = lasso_xb_nms 
          } else {
            xbetas.boot.oob.lasso = rbind( xbetas.boot.oob.lasso, xbetas.lasso.tmp )
            xbetas.boot.inb.lasso = rbind( xbetas.boot.inb.lasso, xbetas.lasso.tmp.tr )
            }
        }
        
        ## calibrate lasso on training data to get ofst (Offset) ##
        if (sum(ensemble[c(2:4,6:8)]) >= 1) { 
          trainpredminR = predict.cv.glmnetr(cv_lasso_fit, trainxs, lambda="lambda.min" , gamma="gamma.min", comment=0 )  ###### min RELAXED lasso as an offset #####
          testpredminR  = predict.cv.glmnetr(cv_lasso_fit, testxs , lambda="lambda.min" , gamma="gamma.min", comment=0 ) 
          ofst0 = trainpredminR 
          if (family=="cox") {
...
#          print( rbind( orf_perf1, orf_perf1_old, orf.cal_perf1 , orf.cal_perf1_ ) )
      ...
      if (doann_==1) { 
        ...
        class(cv_lasso_fit) = "cv.glmnetr"
        ## calibrate lasso ##
        if (sum(ensemble[c(2:8)]) > 0.5) {        
          lassopredtrain0 = predict.cv.glmnetr(cv_lasso_fit, trainxs, lambda="lambda.min" , gamma="gamma.min", comment=0 )
          lassopredtest0  = predict.cv.glmnetr(cv_lasso_fit, testxs , lambda="lambda.min" , gamma="gamma.min", comment=0 )
          lassobeta       = predict.cv.glmnetr(cv_lasso_fit, lambda="lambda.min" , gamma="gamma.min", comment=0 )
        ...
        if (getl1  == 1) { l1  = cv_lasso_fit$relaxed$lambda.min.g0 }          ## CHECK 
        if (getl12 == 1) { l12 = cv_lasso_fit$relaxed$lambda.min.g0 }          ## CHECK   
...
#            print(" HERE 2") ; print(xbetas.cv.ann[oob,][1,])
      ...
      if ((dostep == 1) | (doaic ==1) | (dofull == 1)) { 
        if (bootstrap >= 1) { 
          xbetas.inb.step = matrix(rep(xbnull,4*length(inb)), nrow=length(inb), ncol=4)
          colnames(xbetas.inb.step) = c("step.df", "step.p", "aic", "full")
        } 
      ...
      ##### Full model fit #################################################################################################  
      if (dofull == 1) {
        if (track >= 1) { cat(" ## Starting full model ##\n") } 
        if (family == "cox") {
          if (is.null(start)) { 
            full_fit_i = coxph( Surv(trainy_,trainevent) ~ . , data = data.frame(cbind(trainxs, trainy_, trainevent)) )
#            testxb_full  = predict(full_fit_i, newdata = data.frame(cbind(testxs , testy_ , testevent )) )
#            trainxb_full = predict(full_fit_i, newdata = data.frame(cbind(trainxs, trainy_, trainevent)) )
          } else { 
            full_fit_i = coxph(Surv(trainstart, trainy_, trainevent) ~ . , data = data.frame(cbind(trainxs, trainstart, trainy_, trainevent)) ) 
#            testxb_full  = predict(full_fit_i, newdata = data.frame(cbind(testxs , teststart , testy_ , testevent )) )
#            trainxb_full = predict(full_fit_i, newdata = data.frame(cbind(trainxs, trainstart, trainy_, trainevent)) )
          }
          testxb_full  = predict(full_fit_i, newdata = data.frame(testxs ) )
          trainxb_full = predict(full_fit_i, newdata = data.frame(trainxs) )
        } else { 
          full_fit_i   = glm( trainy_ ~ trainxs )
#          names(full_fit_i)
#          sum(!(colnames(testxs) == colnames(trainxs)))
          trainxb_fullx = predict(full_fit_i, type = "response", newdata = data.frame(trainxs) )
          fullbeta = full_fit_i$coefficients
          fullbeta[is.na(fullbeta)] = 0 
          trainxb_full = cbind(1,trainxs)%*%fullbeta
          testxb_full  = cbind(1,testxs )%*%fullbeta
#          sum(!(trainxb_full == trainxb_full))
#          sum( (trainxb_full == trainxb_full))
#          testxb_full  = predict(full_fit_i, type = "response", newdata = data.frame(testxs ) )
#          produces error  'newdata' had 25 rows but variables found have 75 rows'          
        } 
        full_perf = perf_gen( testy__ , testxb_full , family=family )
        step.devian.rep[i_ , 4] = c( full_perf[1] )
        step.agree.rep[i_, 4]   = c( full_perf[3] )
        step.nzero.rep[i_, 4]   = sum(!is.na(full_fit_i$coefficients))

        xbetas.cv.step[ oob ,4] = testxb_full
        if (bootstrap >= 1) { xbetas.inb.step[,4] = trainxb_full } 
      ...
      ## end within fold Step, AIC , Full Fits #################################################
...
#' @noRd
#'
nested_tolist = function(Call, family, xs, start, y_, y__, event, ties, id, start_name, y_name, event_name,  
                        seed, foldid, ensemble, folds_n, alpha, gamma, lambda, stratified, limit, fine, method, steps_n, 
                        bootstrap, bootstrap_, unique, resample, keepdata, keepxbetas, track, 
                        dolasso, doxgb, doxgb_, dorf, dorf_, doorf, doorf_, dorpart, doann, doann_, dostep, doaic, dofull, 
                        xbetas, xbetas.cv, xbetas.boot.oob.null, xbetas.boot.inb.null, 
  ...
  if (bootstrap == 0) {
    ...
    if ((dostep == 1) | (doaic==1) | (dofull == 1)) { xbetas.cv = cbind( xbetas.cv, xbetas.cv.step[,keepxbetas.step, drop=FALSE]) } 
  } else {
  ...
  nestedcv = list( call = Call, 
                   ...
                   fits   = c( dolasso=dolasso, doxgb=doxgb_, dorf=dorf_, dorpart=dorpart, doann=doann_, dostep=dostep, doaic=doaic, doorf=doorf_, dofull=dofull), 
                   alpha  = alpha, 
                   gamma  = gamma, 
                   lambda = lambda, 
                   seed   = seed, 
  ...
  if (dolasso == 1) { 
    ...
    nestedcv$cv_lasso_fit       = cv_lasso_fit_f 
    nestedcv$cv_ridge_fit       = cv_ridge_fit_f 
    if (length(alpha) > 1) { 
      nestedcv$cv_elastic_fit   = cv_elastic_fit_f 
      nestedcv$cv_glmnet_fit_   = cv_glmnet_fit_f_ 
    } 
  } 
