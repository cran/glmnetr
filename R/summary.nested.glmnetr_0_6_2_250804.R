################################################################################  
#' Summarize a nested.glmnetr() output objects version 0.6-1
#'
#' @description 
...
#  w = colSums ( n.rep*( null.m2LogLik.rep - devian.rep )) / ( as.numeric(object$sample[2]) * as.numeric(object$sample[6])) # sum((null.m2LogLik.rep - sat.m2LogLik.rep)*n.rep) ## wont work for Cox 
  ...
  if (cvfit==TRUE) {
    summary.cv.glmnetr_0_6_1(object=object, type=type, printg1=printg1, orderall=FALSE) 
  } else {
    #---------------------------------------------------------------------------------------
...
#      print(x_1)
    ...
    last = "none"
    if (dofull == 1) { last = "full"  
    } else if (doaic  == 1) { last = "aic"  
...
#        xgb
    ...
    ##### STEPWISE p or df tuned & AIC & full ##################################
...
#        names( step ) = c("Deviance", "Inter", "Slope", "Concor", "Non Zero", "Deviancd", "ConroR", "Non Zero" )
      ...
      row.names(step) = c("Stepwise df tuned          ", "Stepwise p tuned       ", "Stepwise AIC             ", "Full regression    " )
      step = step[ c((dostep == 1),(dostep == 1),(doaic == 1),(dofull==1)) , ]
