...
#'
summary.nested.glmnetr = function(object, cvfit=FALSE, pow=2, printg1=FALSE, short=0, digits=3, Call=NULL, ...) {
...
#  print(xgb.agree.cv)
  ...
  if ((doxgb == 1) & (dorf == 1)) {
    ...
    if (sum(ensemble[c(2,6)]) > 0) {
      cat ("\n XGBoost - RF with lasso feature             ") ;  glmnetr.compcv0(xgb.agree.cv[,5] ,  rf.agree.cv[,2],pow=pow)   
    } 
    if ( (sum(ensemble[c(3,4,7,8)]) > 0)  & (family == "gaussian") ) {
      cat ("\n XGBoost - RF with lasso offset              ") ;  glmnetr.compcv0(xgb.agree.cv[,6] ,  rf.agree.cv[,3],pow=pow)   
    }
  }
  ...
  if ((doxgb == 1) & (doann == 1)) {
    ...
    if (ensemble[6]) { 
      cat ("\n XGBoost - ANN, lasso feature                ") ;  glmnetr.compcv0(xgb.agree.cv[,5] ,  ann.agree.cv[,6],pow=pow)   
    } else if (ensemble[2]) { 
      cat ("\n XGBoost - ANN, lasso feature                ") ;  glmnetr.compcv0(xgb.agree.cv[,5] ,  ann.agree.cv[,2],pow=pow)   
    } 
    if (family == "gaussian") {
      if (ensemble[8]) { 
        cat ("\n XGBoost - ANN, l lasso offset (upated)      ") ;  glmnetr.compcv0(xgb.agree.cv[,6] ,  ann.agree.cv[,8],pow=pow)   
      } else if (ensemble[4]) { 
        cat ("\n XGBoost - ANN, lasso offset                 ") ;  glmnetr.compcv0(xgb.agree.cv[,6] ,  ann.agree.cv[,4],pow=pow)   
      } else if (ensemble[7]) { 
        cat ("\n XGBoost - ANN, l lasso offset (updated)     ") ;  glmnetr.compcv0(xgb.agree.cv[,6] ,  ann.agree.cv[,7],pow=pow)   
      } else if (ensemble[3]) { 
        cat ("\n XGBoost - ANN, lasso offset                 ") ;  glmnetr.compcv0(xgb.agree.cv[,6] ,  ann.agree.cv[,3],pow=pow)   
      }  
    }
  }
  ...
  if ((dorf == 1) & (doann == 1)) {
    ...
    if (ensemble[6]) {
      cat ("\n RF - ANN, l lasso feature                   " ) ;  glmnetr.compcv0(rf.agree.cv[,2] ,  ann.agree.cv[,6],pow=pow) 
    } else if (ensemble[2]) {
      cat ("\n RF - ANN, lasso feature                     " ) ;  glmnetr.compcv0(rf.agree.cv[,2] ,  ann.agree.cv[,2],pow=pow)  
    }
    if (ensemble[8]) {
      cat ("\n RF - ANN, l lasso offset (upated)           " ) ;  glmnetr.compcv0(rf.agree.cv[,3] ,  ann.agree.cv[,8],pow=pow)   
    } else if (ensemble[4]) {
      cat ("\n RF - ANN, lasso offset                      " ) ;  glmnetr.compcv0(rf.agree.cv[,3] ,  ann.agree.cv[,4],pow=pow)  
    } else if (ensemble[7]) {
      cat ("\n RF - ANN, l lasso offset (upated)           " ) ;  glmnetr.compcv0(rf.agree.cv[,3] ,  ann.agree.cv[,7],pow=pow) 
    } else if (ensemble[3]) {
      cat ("\n RF - ANN, lasso offset                      " ) ;  glmnetr.compcv0(rf.agree.cv[,3] ,  ann.agree.cv[,3],pow=pow)  
    }
    cat("\n")
  }
