...
library(rpart)
library(torch)
source("~/Documents/LASSO/glmnetr/R/aicreg_230218.R"              , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/ann_tab_cv_230509.R"          , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/cv.glmnetr_230508.R"          , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/cv.stepreg_230218.R"          , echo=TRUE)
#source("~/Documents/LASSO/glmnetr/R/globalVariables_230509.R"     , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/nested.glmnetr_230508.R"      , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/plot.cv.glmnetr_230508.R"     , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/predict_ann_tab_230508.R"     , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/stepreg_230218.R"             , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/summary.cv.glmnetr_230508.R"  , echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/summary.nested.glmnetr_230508.R", echo=TRUE)
source("~/Documents/LASSO/glmnetr/R/xgbm_tuned_230322.R"          , echo=TRUE)
```
# The functions
...
ann_tab_cox_ex1 = ann_tab_cv(myxs=xs, myy=yt, myevent=event, family="cox", fold_n=10, eppr=-2)  
```
...
lenx_
```
From this we also see the number of predictors to be `r lenx_`.  As we have implemented this lasso informed fit we only include those terms that had non zero coefficients in the tuned relaxed lasso model.  We can inspect the model structure as in 
...
preds = predict_ann_tab(nested_cox_fit_ex2, xs, modl=3) 
preds[1:10]
```

Here the first input is the nested.glmnetr() output object, the second input the data for which we want the predicteds and finally modl=3 means we want the predicted from the model indicated by ensemble[3] = 1 in the call to nested.glmentr().  
...
nested_nrm_fit_ex2 = nested.glmnetr(xs=xs, y_=y_, family="gaussian", dolasso=1,
        seed=c(17820414,95337508), doann=1, ensemble=c(1,0,1), folds_n=c(10,0), track=-1)
```
We can inspect numerically the fit by 
```{r  nested.glminetr() ann_fit_1 predicteds 2 }
preds = predict_ann_tab(nested_nrm_fit_ex2, xs, modl=3) 
...
nested_bin_fit_ex4 = suppressWarnings( nested.glmnetr(xs=xs, y_=yb, family="binomial", 
                     dolasso=1, doann=1, ensemble=c(1,0,1), folds_n=c(5,0), 
                     track=-1) )
...
ensemble=c(1,0,1,0,0) ;
doann=list(epochs=200, epochs2=400, mylr=0.002, mylr2=0.001, eppr=-2, eppr2=-2, 
          lenz1=16, lenz2=8, actv=1, drpot=0, wd=0, wd2=0, L1=0, L12=0, 
          fold_n=0, minloss=0, gotoend=0) 
nested_nrm_fit_ex5 = nested.glmnetr(xs=simdata$xs,y_=simdata$y_,
                    family="gaussian",dolasso=1,doann=doann,
                    ensemble=ensemble,folds_n=c(8,1),steps_n=40,track=-1)
## print a short summary 
nested_nrm_fit_ex5
```
where the NN model with an R-square of 0.91 seems to out perform the lasso models with an R-squares of about 0.84.  Further, the NN fit without this transfer of information from the lasso model with it's R-square of 0.70, did not perform nearly as well as the lasso informed NN or the lasso model itself.  This shows the direct benefit of this transfer learning of the information from the linear model when fitting NN models.  It is not the NN itself that it is outperforming the lasso model but the NN model fit in conjunction with the lasso model information.  We can further inspect performances with the summary function as in 
