...
calplot(nested.cox.fit, wbeta=3, resample=0, xlim=c(-10,9), ylim=c(-15,10), col.term=2, col.se=7)
```
...
nested.cox.fit$xbetas.cv = nested.cox.fit$xbetas.cv[,c(1:6,22,23)] 
```
```{r calplot 3 }
...
calplot(nested.bin.fit, 4, plotfold=0) 
```
...
calplot(nested.bin.boot.fit, 3, plotfold=1) 
```
...
calplot(nested.bin.boot.fit, wbeta=12, bootci=0, oob=0, plotfold=1) 
```
...
calplot(nested.bin.boot.fit, wbeta=12, bootci=1, plotfold=1)
```
...
calplot(nested.bin.boot.fit, wbeta=12, plotfold=1)
```
...
calplot(nested.gau.fit, wbeta=4)
```
