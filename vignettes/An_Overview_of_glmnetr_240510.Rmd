...
knitr::opts_chunk$set(echo=TRUE)
 library( glmnetr )
#source("~/Documents/Rpackages/glmnetr_supl/_load_glmnetr_vig_240510.R" , echo=TRUE)
```
...
load( file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_240510.RLIST" ) 
```
...
nested.gau.fit = nested.glmnetr(xs,NULL,y_,NULL,family="gaussian",
      ...
      ensemble=1, folds_n=10, seed=219301029, track=1) 
```
...
nested.cox.fit2 = nested.glmnetr(xs,strt,yt,event,family="cox",
      dolasso=1, doxgb=list(nrounds=250), dorf=1, doorf=1, doann=1, dorpart=1, 
      ensemble=1, folds_n=10, seed=219301029, track=1) 
```
...
```
From this spline fit we see the log hazard ratio is roughly linear in the relaxed lasso predicteds.  Still, a calibration based upon data not used in model derivation would be more meaningful.  The calplot() function does this fitting splines for each of the hold out data sets of the outer loop of the nested cross validation, for example
```{r calplot 1}
...
save( nested.cox.fit , nested.cox.fit2, nested.gau.fit , nested.bin.fit , file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_240510.RLIST" ) 
```