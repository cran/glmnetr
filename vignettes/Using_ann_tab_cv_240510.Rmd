...
library( glmnetr )
#source("~/Documents/Rpackages/glmnetr_supl/_load_glmnetr_vig_240510.R" , echo=FALSE)
```
# The functions
...
load( file="~/Documents/Rpackages/glmnetr_supl/using_ann_tab_cv_outputs_240510.RLIST" ) 
```
...
```
Here since we did a nested cross validation of the derived model (with the option do_ncv left as defualt) we can calibrate using the calplot() function 
...
Since we did the nested cross validation (by leaving the do_ncv option as default) when generating nested_nrm_fit_ex5 we can draw a calibration plot using the calplot() function. For the "basic" ANN this is 
```{r calplot 1}
...
save( ann_tab_cox_ex1, nested_cox_fit_ex2, nested_nrm_fit_ex3, nested_bin_fit_ex4 , nested_nrm_fit_ex5 , 
      file="~/Documents/Rpackages/glmnetr_supl/using_ann_tab_cv_outputs_240510.RLIST") 
```