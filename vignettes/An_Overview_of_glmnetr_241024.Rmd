...
load( file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_241024.RLIST" ) 
```
...
save( nested.cox.fit , 
      file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_241024.RLIST" ) 
```
...
```
Here, similar to the cross-validation calibration plots, calibration curves are fit using splines for each bootstrap sample based upon the upon the XBeta's (predicteds) for the out-of-bag sample based upon the re-sample fitted model. These curves are then averaged to get an overall calibration curve. 95% confidence intervals are described based upon the overall curve and the standard deviation of the individual curves at each value of XBeta. See the calibration vignette for more on calibration curves based upon bootstrap samples. 
