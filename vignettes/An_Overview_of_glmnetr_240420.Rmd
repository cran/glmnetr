...
source("~/Documents/Rpackages/glmnetr_supl/_load_glmnetr_vig_240409.R" , echo=TRUE)
```
# The Package 
...
nested.cox.fit2 = nested.glmnetr(xs,strt,yt,event,family="cox",
      dolasso=1, doxgb=list(nrounds=250), dorf=1, doann=1, dorpart=1, ensemble=c(1,0,0,0, 0,1,0,1), 
      folds_n=10, seed=219301029, track=1) 
```

```{r  Nested cross-validation evaluation for multiple machine learning models - binomial check, include=FALSE, eval=evalmodel}
nested.bin.fit
```
...
\newpage
To understand how the models might be over or underestimating the strength of the association between the preditors and outcome one can regress the outcome on the model X*beta predicteds, i.e. to a linear calibration.  Slope terms from this linear calibration can be plotted as in 
...
print(fit1)
```
More ideas for calibration can be found in the "Calibration" vignette.  
\newpage
```{r Calibration plot termplot}
termplot(fit1,term=1,se=TRUE)
```
From this spline fit we see the log hazard ratio is roughly linear in the relaxed lasso predicteds.  Still, a calibration badsed upon data not used in model derivation would be stronger.  Calibration is discussed in more detail in the "Calibration of Machine Learning Models" vignette. 
...
glmnetr.compcv(nested.gau.fit, type="devrat")
```   
...
save( nested.cox.fit , nested.cox.fit2, nested.gau.fit , nested.bin.fit , file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_240412.RLIST" ) 
```