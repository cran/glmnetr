...
nested.cox.fit = nested.glmnetr(xs, NULL, yt, event, family="cox", 
      ...
      alpha=seq(0,1,0.2), folds_n=10, steps_n=40, track=1) 
```
```{r Fit a relaxed lasso model informed by cross validation 2, include=TRUE, eval=FALSE}
set.seed(465783345) 
nested.cox.fit = nested.glmnetr(xs, NULL, yt, event, family="cox", 
                 dolasso=1, alpha=seq(0,1,0.2), dostep=1, steps_n=40, folds_n=10, track=1) 
```
```{r save1, include=FALSE, eval=evalmodel}
save( nested.cox.fit , 
      file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_250818.RLIST" ) 
```
...
#nested.cox.fit$fits[4] = 1
```
```{r values1, include=FALSE}
lambda.min = round(log(nested.cox.fit$cv_lasso_fit$relaxed$lambda.min),digits=2)
lambda.min.nzero = nested.cox.fit$cv_lasso_fit$nzero[nested.cox.fit$relaxed$index[1,1]]
gamma.min = nested.cox.fit$cv_lasso_fit$relaxed$gamma.min
lambda.min.g0 = round(log(nested.cox.fit$cv_lasso_fit$relaxed$lambda.min.g0),digits=2)
lambda.min.g0.nzero=nested.cox.fit$cv_lasso_fit$nzero[nested.cox.fit$cv_lasso_fit$relaxed$index.g0[1]]
lambda.min.R.nzero =nested.cox.fit$cv_lasso_fit$nzero[nested.cox.fit$cv_lasso_fit$relaxed$index[1,1]]
lambda.min.g1.nzero=nested.cox.fit$cv_lasso_fit$nzero[nested.cox.fit$cv_lasso_fit$index[1]]
```
...
save( nested.cox.fit , nested.gau.fit , nested.bin.fit ,  
      file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_250818.RLIST" ) 
```
```{r Bootstrap evaluation for multiple machine learning models - binomial - boot, include=FALSE, eval=evalmodel}
nested.bin.boot.fit = nested.glmnetr(xs,NULL,yb,NULL,family="binomial",
      dolasso=1, dorf=1, doorf=1, doann=list(bestof=10),   
      folds_n=10, seed=219301029, bootstrap=20, track=1) 
nested.bin.boot.fit.1 = nested.glmnetr(xs,NULL,yb,NULL,family="binomial",
      dolasso=1, dorf=1, doorf=1,   
      folds_n=10, seed=219301029, bootstrap=1, track=1) 
#nested.bin.boot.fit
```
```{r save4, include=FALSE, eval=evalmodel}
save( nested.cox.fit , nested.gau.fit , nested.bin.fit , nested.bin.boot.fit, nested.bin.boot.fit.1,  
      file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_250818.RLIST" ) 
```
...
save( nested.cox.fit , nested.gau.fit , nested.bin.fit , nested.bin.boot.fit , nested.bin.boot.fit.1, nested.cox.fit2 , 
      file="~/Documents/Rpackages/glmnetr_supl/using_glmnetr_outputs_250818.RLIST" ) 
```
...
```
Here the "overall" calibration and confidence interval is based upon the average and +/- 2 standard error calculated using the usual formula for the mean and the values from the different folds. The folds though are not independent and so these formula may be off. One can also include the individual calibration curves from the outer cross validation loop with the option plotfold=1 as in 
...
Bengio et al. showed that the standard deviations from cross validation might not be accurate for estimation of the actual standard errors. Bates et al. have shown that the cross validation (CV) estimates and standard errors may be biased, and should thus be viewed with caution. These considerations regard the cross valitiona for evalation of model performance, here the outer loop, and are agnostic to whether or not the models use any cross validation fore the derivation of the models, here the inner loop. In particular the CV estimates model performance may more closely estimate the expection over multiple samples than the performance of the model based upon the observed data. In this manner the CV estimates may be reasonable estimates for the model fitting process, but may be biased for a particular model fit. Further, with their bias the CV standard errors might be associated with mis-coverage three times the nominal mis-coverage. A resolution of these biases in estimation and standard error proposed by Bates at al. involves two more iteration loops lengthening computation time by a factor of about 10 when using 10-fold cross validation and possibly by a factor of 100 if one is to replicate the whole process multiple times to get more accurate estimates. This may be practical for machine learning models that run more quickly like possibly lasso and random forests but impractical for models that can take much longer like tuned gradient boosting machines. This is not (yet?) implemented here.
...
```{r bootstrap perform plot pre, include=FALSE, eval=TRUE}
nested.bin.boot.fit$fits[5] = 0 
```  
```{r bootstrap perform plot, include=TRUE, eval=TRUE}
plot(nested.bin.boot.fit)
```  
```{r bootstrap perform plot post, include=FALSE, eval=TRUE}
nested.bin.boot.fit$fits[5] = 1 
```  
Calibration curves can be generated like with the cross validation model evaluations, as in 
```{r bootstrap plot pre, include=TRUE, eval=TRUE}
calplot(nested.bin.boot.fit, wbeta=3, plotfold=1)  
```
Here, similar to the cross-validation calibration plots, calibration curves are fit using splines for each bootstrap sample based upon the upon the XBeta's (predicteds) for the out-of-bag sample based upon the re-sample fitted model. These curves are then averaged to get an overall calibration curve. Nominal 95% confidence intervals are described based upon the overall curve and the standard deviation of the individual curves at each value of XBeta, though again the coverage of these nominal confidence intervals is in question. See the calibration vignette for more on calibration curves based upon bootstrap samples.
...
Riley RD, Pate A, Dhiman P, Archer L, Martin GP, Collins GS, "Clinical prediction models and the multiverse of madness", BMC Med 21, 502 (2023). https://doi.org/10.1186/s12916-023-03212-y 
...
```{r cross validation bin calplot, include=FALSE, eval=FALSE}
# for inclusion in calibration vignette 
#calplot(nested.bin.fit, wbeta=3, plotfold=1) 
```
```{r save, include=FALSE, eval=evalmodel}
# duplicate of {r save5, ... }
